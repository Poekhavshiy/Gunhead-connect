...
<#
.SYNOPSIS
    Simple working build script for Gunhead-Connect
...
function Show-Help {
    Write-ColorText "Gunhead-Connect Simple Build Script" $BLUE
    Write-ColorText ""
    Write-ColorText "Usage: .\simple-build.ps1 [options]" $GREEN
    Write-ColorText ""
...
if (-not ($JustBuild -or $BuildRun -or $Run -or $Clean)) {
    Show-Help
}
...
try {
    ...
    if ($JustBuild -or $BuildRun) {
        ...
        # Configure - use MSYS Makefiles which should work
        & $CMAKE_PATH -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_PREFIX_PATH="D:\Qt\6.9.2\mingw_64" -B $BUILD_DIR -G "MSYS Makefiles"
        ...
        # Use cmake --build which should work with any generator
        & $CMAKE_PATH --build $BUILD_DIR --config $BUILD_TYPE --target Gunhead-Connect
        
        if ($LASTEXITCODE -ne 0) {
            Write-ColorText "Main target failed, trying full build..." $YELLOW
        ...
        # Copy compile commands if they exist
        if (Test-Path "$BUILD_DIR\compile_commands.json") {
            if (-not (Test-Path "buildfiles")) {
                New-Item -ItemType Directory -Path "buildfiles" -Force | Out-Null
            }
            Copy-Item "$BUILD_DIR\compile_commands.json" "buildfiles\compile_commands.json" -Force
            Write-ColorText "Compile commands exported!" $GREEN
        }
    }
